    ...
    <div class="mx-auto max-w-5xl" x-data="{ tab: 'details' }">
        <div class="mb-8 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
            <div>
                ...
                <h1 class="mt-3 text-3xl font-bold text-slate-900">{{ $labMedia->verification_no }}</h1>
                <p class="mt-2 text-sm text-slate-500">{{ $labMedia->media_name }} · {{ $labMedia->verification_date?->format('d M Y') }}</p>
            </div>
            <div class="flex items-center gap-3">
                <x-lab-status-badge :status="$labMedia->status" class="!px-3 !py-1.5 !text-xs" />
                @can('update', $labMedia)<a href="{{ route('lab-media.edit', $labMedia) }}" class="rounded-xl bg-blue-600 px-4 py-2.5 text-sm font-semibold text-white shadow-sm hover:bg-blue-700">Edit</a>@endcan
            </div>
        </div>
        ...
        <div x-show="tab === 'details'" class="grid gap-6 sm:grid-cols-2">
            <div class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm sm:col-span-2">
                <h2 class="mb-4 text-sm font-bold uppercase tracking-wide text-slate-400">Stock detail</h2>
                <dl class="grid gap-3 text-sm sm:grid-cols-2">
                    <div class="flex justify-between"><dt class="text-slate-500">Verified by</dt><dd class="font-medium text-slate-800">{{ $labMedia->verifiedBy?->name ?? '—' }}</dd></div>
                    <div class="flex justify-between"><dt class="text-slate-500">Opening stock</dt><dd class="font-medium text-slate-800">{{ $labMedia->opening_stock }} {{ $labMedia->unit }}</dd></div>
                    <div class="flex justify-between"><dt class="text-slate-500">Received</dt><dd class="font-medium text-slate-800">{{ $labMedia->received_qty }} {{ $labMedia->unit }}</dd></div>
                    <div class="flex justify-between"><dt class="text-slate-500">Consumed</dt><dd class="font-medium text-slate-800">{{ $labMedia->consumed_qty }} {{ $labMedia->unit }}</dd></div>
                    <div class="flex justify-between"><dt class="text-slate-500">Closing stock</dt><dd class="font-medium text-slate-800">{{ $labMedia->closing_stock }} {{ $labMedia->unit }}@if($labMedia->is_low_stock)<span class="ml-2 rounded-full bg-rose-100 px-2 py-0.5 text-xs font-semibold text-rose-700">Low stock</span>@endif</dd></div>
                    <div class="flex justify-between"><dt class="text-slate-500">Reorder level</dt><dd class="font-medium text-slate-800">{{ $labMedia->reorder_level ?? '—' }}</dd></div>
                </dl>
                @if($labMedia->remarks)<p class="mt-4 text-sm text-slate-700">{{ $labMedia->remarks }}</p>@endif
            </div>

            @if($labMedia->photo)
                <div class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm sm:col-span-2">
                    <h2 class="mb-4 text-sm font-bold uppercase tracking-wide text-slate-400">Photo evidence</h2>
                    <a href="{{ \Illuminate\Support\Facades\Storage::url($labMedia->photo) }}" target="_blank"><img src="{{ \Illuminate\Support\Facades\Storage::url($labMedia->photo) }}" class="max-h-72 rounded-xl border border-slate-200 object-cover"></a>
                </div>
            @endif
            ...
            <div class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm sm:col-span-2">
                ...
                <dl class="grid gap-3 text-sm sm:grid-cols-2">
                    <div class="flex justify-between"><dt class="text-slate-500">Created by</dt><dd class="font-medium text-slate-800">{{ $labMedia->createdBy?->name ?? '—' }} · {{ $labMedia->created_at?->format('d M Y') }}</dd></div>
                    <div class="flex justify-between"><dt class="text-slate-500">Last updated by</dt><dd class="font-medium text-slate-800">{{ $labMedia->updatedBy?->name ?? '—' }} · {{ $labMedia->updated_at?->format('d M Y') }}</dd></div>
                </dl>
            </div>
        </div>
        ...
        <div x-show="tab === 'approval'" x-cloak class="space-y-6">
            @can('submit', $labMedia)
                <div class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
                    <p class="text-sm text-slate-600">This verification is still a draft. Submit it for supervisor approval once it's complete.</p>
                    <form method="POST" action="{{ route('lab-media.submit', $labMedia) }}" class="mt-4">@csrf<button class="rounded-xl bg-blue-600 px-5 py-2.5 text-sm font-semibold text-white shadow-sm hover:bg-blue-700">Submit for approval</button></form>
                </div>
            @endcan

            @can('approve', $labMedia)
                <div class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
                    <h2 class="mb-4 text-sm font-bold uppercase tracking-wide text-slate-400">Supervisor decision</h2>
                    <form method="POST" action="{{ route('lab-media.approve', $labMedia) }}" class="space-y-4" onsubmit="return window.arcadiaCheckSignaturePresent(this, 'media-approve-pad_input')">
                        @csrf
                        ...
                        <x-lab-signature-pad id="media-approve-pad" />
                    ...
                    <form method="POST" action="{{ route('lab-media.reject', $labMedia) }}" class="mt-4 space-y-4 border-t border-slate-100 pt-4">
                        @csrf
            ...
            @can('unlock', $labMedia)
                <div class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
                    <p class="text-sm text-slate-600">Reopen this verification for editing.</p>
                    <form method="POST" action="{{ route('lab-media.unlock', $labMedia) }}" class="mt-4">@csrf<button class="rounded-xl bg-amber-500 px-5 py-2.5 text-sm font-semibold text-white shadow-sm hover:bg-amber-600">Unlock</button></form>
                </div>
            @endcan
