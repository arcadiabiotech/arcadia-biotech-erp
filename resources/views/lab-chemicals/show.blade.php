    ...
    <div class="mx-auto max-w-5xl" x-data="{ tab: 'details' }">
        <div class="mb-8 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
            <div>
                ...
                <h1 class="mt-3 text-3xl font-bold text-slate-900">{{ $labChemical->usage_no }}</h1>
                <p class="mt-2 text-sm text-slate-500">{{ $labChemical->chemical_name }} · {{ $labChemical->usage_date?->format('d M Y') }}</p>
            </div>
            <div class="flex items-center gap-3">
                <x-lab-status-badge :status="$labChemical->status" class="!px-3 !py-1.5 !text-xs" />
                @can('update', $labChemical)<a href="{{ route('lab-chemicals.edit', $labChemical) }}" class="rounded-xl bg-blue-600 px-4 py-2.5 text-sm font-semibold text-white shadow-sm hover:bg-blue-700">Edit</a>@endcan
            </div>
        </div>
        ...
        <div x-show="tab === 'details'" class="grid gap-6 sm:grid-cols-2">
            <div class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm sm:col-span-2">
                <h2 class="mb-4 text-sm font-bold uppercase tracking-wide text-slate-400">Usage detail</h2>
                <dl class="grid gap-3 text-sm sm:grid-cols-2">
                    <div class="flex justify-between"><dt class="text-slate-500">Used by</dt><dd class="font-medium text-slate-800">{{ $labChemical->usedBy?->name ?? '—' }}</dd></div>
                    <div class="flex justify-between"><dt class="text-slate-500">Quantity used</dt><dd class="font-medium text-slate-800">{{ $labChemical->quantity_used }} {{ $labChemical->unit }}</dd></div>
                    <div class="flex justify-between"><dt class="text-slate-500">Batch number</dt><dd class="font-medium text-slate-800">{{ $labChemical->batch_number ?? '—' }}</dd></div>
                    <div class="flex justify-between"><dt class="text-slate-500">Purpose</dt><dd class="font-medium text-slate-800">{{ $labChemical->purpose ?? '—' }}</dd></div>
                </dl>
                @if($labChemical->remarks)<p class="mt-4 text-sm text-slate-700">{{ $labChemical->remarks }}</p>@endif
            </div>

            @if($labChemical->photo)
                <div class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm sm:col-span-2">
                    <h2 class="mb-4 text-sm font-bold uppercase tracking-wide text-slate-400">Photo evidence</h2>
                    <a href="{{ \Illuminate\Support\Facades\Storage::url($labChemical->photo) }}" target="_blank"><img src="{{ \Illuminate\Support\Facades\Storage::url($labChemical->photo) }}" class="max-h-72 rounded-xl border border-slate-200 object-cover"></a>
                </div>
            @endif
            ...
            <div class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm sm:col-span-2">
                ...
                <dl class="grid gap-3 text-sm sm:grid-cols-2">
                    <div class="flex justify-between"><dt class="text-slate-500">Created by</dt><dd class="font-medium text-slate-800">{{ $labChemical->createdBy?->name ?? '—' }} · {{ $labChemical->created_at?->format('d M Y') }}</dd></div>
                    <div class="flex justify-between"><dt class="text-slate-500">Last updated by</dt><dd class="font-medium text-slate-800">{{ $labChemical->updatedBy?->name ?? '—' }} · {{ $labChemical->updated_at?->format('d M Y') }}</dd></div>
                </dl>
            </div>
        </div>

        <div x-show="tab === 'approval'" x-cloak class="space-y-6">
            @can('submit', $labChemical)
                <div class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
                    <p class="text-sm text-slate-600">This entry is still a draft. Submit it for supervisor approval once it's complete.</p>
                    <form method="POST" action="{{ route('lab-chemicals.submit', $labChemical) }}" class="mt-4">@csrf<button class="rounded-xl bg-blue-600 px-5 py-2.5 text-sm font-semibold text-white shadow-sm hover:bg-blue-700">Submit for approval</button></form>
                </div>
            @endcan

            @can('approve', $labChemical)
                <div class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
                    <h2 class="mb-4 text-sm font-bold uppercase tracking-wide text-slate-400">Supervisor decision</h2>
                    <form method="POST" action="{{ route('lab-chemicals.approve', $labChemical) }}" class="space-y-4" onsubmit="return window.arcadiaCheckSignaturePresent(this, 'chemical-approve-pad_input')">
                        @csrf
                        ...
                        <x-lab-signature-pad id="chemical-approve-pad" />
                    ...
                    <form method="POST" action="{{ route('lab-chemicals.reject', $labChemical) }}" class="mt-4 space-y-4 border-t border-slate-100 pt-4">
                        @csrf
            ...
            @can('unlock', $labChemical)
                <div class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
                    <p class="text-sm text-slate-600">Reopen this entry for editing.</p>
                    <form method="POST" action="{{ route('lab-chemicals.unlock', $labChemical) }}" class="mt-4">@csrf<button class="rounded-xl bg-amber-500 px-5 py-2.5 text-sm font-semibold text-white shadow-sm hover:bg-amber-600">Unlock</button></form>
                </div>
            @endcan
