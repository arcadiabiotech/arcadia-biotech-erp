    ...
    <div class="mx-auto max-w-5xl" x-data="{ tab: 'details' }">
        <div class="mb-8 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
            <div>
                ...
                <h1 class="mt-3 text-3xl font-bold text-slate-900">{{ $labChecklist->checklist_no }}</h1>
                <p class="mt-2 text-sm text-slate-500">{{ $labChecklist->employee?->name }} · {{ $labChecklist->checklist_date?->format('d M Y') }}</p>
            </div>
            <div class="flex items-center gap-3">
                <x-lab-status-badge :status="$labChecklist->status" class="!px-3 !py-1.5 !text-xs" />
                @can('update', $labChecklist)<a href="{{ route('lab-checklists.edit', $labChecklist) }}" class="rounded-xl bg-blue-600 px-4 py-2.5 text-sm font-semibold text-white shadow-sm hover:bg-blue-700">Edit</a>@endcan
            </div>
        </div>
        ...
        @php
            $existingItems = $labChecklist->checklistItems->keyBy('item_key');
        ...
        <div x-show="tab === 'details'" class="grid gap-6 sm:grid-cols-2">
            <div class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm sm:col-span-2">
                <div class="flex items-center justify-between">
                    <h2 class="text-sm font-bold uppercase tracking-wide text-slate-400">Compliance</h2>
                    <span class="text-lg font-bold text-slate-900">{{ $labChecklist->compliance_score }}%</span>
                </div>
                @if($labChecklist->general_remarks)<p class="mt-4 text-sm text-slate-700">{{ $labChecklist->general_remarks }}</p>@endif
            </div>

            <div class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm sm:col-span-2">
                <h2 class="mb-4 text-sm font-bold uppercase tracking-wide text-slate-400">Morning checklist</h2>
                    ...
                    @foreach($morningItems as $key => $meta)
                        <div class="flex items-center justify-between border-b border-slate-100 pb-2 last:border-0 last:pb-0">
                            <dt class="flex items-center gap-2 text-slate-600">
                                <span @class(['h-2 w-2 rounded-full', 'bg-emerald-500' => $existingItems[$key]->is_done ?? false, 'bg-slate-300' => ! ($existingItems[$key]->is_done ?? false)])></span>
                                {{ $meta['label'] }}
                            </dt>
                            <dd class="font-medium text-slate-800">{{ $existingItems[$key]->time_recorded ?? '—' }}</dd>
                        </div>
                    @endforeach
                </dl>
            </div>
            ...
            <div class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm sm:col-span-2">
                <h2 class="mb-4 text-sm font-bold uppercase tracking-wide text-slate-400">Daily report</h2>
                <dl class="space-y-2 text-sm">
                    @foreach($dailyReportItems as $key => $meta)
                        <div class="flex items-center justify-between border-b border-slate-100 pb-2 last:border-0 last:pb-0">
                            <dt class="flex items-center gap-2 text-slate-600">
                                <span @class(['h-2 w-2 rounded-full', 'bg-emerald-500' => $existingItems[$key]->is_done ?? false, 'bg-slate-300' => ! ($existingItems[$key]->is_done ?? false)])></span>
                                {{ $meta['label'] }}
                                @if($meta['chemical'] && ($existingItems[$key]->chemical_used ?? null))
                                    <span class="text-xs text-slate-400">({{ $existingItems[$key]->chemical_used }})</span>
                                @endif
                            </dt>
                            <dd class="font-medium text-slate-800">{{ $existingItems[$key]->time_recorded ?? '—' }}</dd>
                        </div>
            ...
            @if($labChecklist->photo)
                <div class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm sm:col-span-2">
                    ...
                    <a href="{{ \Illuminate\Support\Facades\Storage::url($labChecklist->photo) }}" target="_blank"><img src="{{ \Illuminate\Support\Facades\Storage::url($labChecklist->photo) }}" class="max-h-72 rounded-xl border border-slate-200 object-cover"></a>
                </div>
            @endif
            ...
            <div class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm sm:col-span-2">
                ...
                <dl class="grid gap-3 text-sm sm:grid-cols-2">
                    <div class="flex justify-between"><dt class="text-slate-500">Created by</dt><dd class="font-medium text-slate-800">{{ $labChecklist->createdBy?->name ?? '—' }} · {{ $labChecklist->created_at?->format('d M Y') }}</dd></div>
                    <div class="flex justify-between"><dt class="text-slate-500">Last updated by</dt><dd class="font-medium text-slate-800">{{ $labChecklist->updatedBy?->name ?? '—' }} · {{ $labChecklist->updated_at?->format('d M Y') }}</dd></div>
                </dl>
            </div>
        </div>

        <div x-show="tab === 'approval'" x-cloak class="space-y-6">
            @can('submit', $labChecklist)
                <div class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
                    <p class="text-sm text-slate-600">This checklist is still a draft. Submit it for supervisor approval once it's complete.</p>
                    <form method="POST" action="{{ route('lab-checklists.submit', $labChecklist) }}" class="mt-4">@csrf<button class="rounded-xl bg-blue-600 px-5 py-2.5 text-sm font-semibold text-white shadow-sm hover:bg-blue-700">Submit for approval</button></form>
                </div>
            @endcan

            @can('approve', $labChecklist)
                <div class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
                    <h2 class="mb-4 text-sm font-bold uppercase tracking-wide text-slate-400">Supervisor decision</h2>
                    <form method="POST" action="{{ route('lab-checklists.approve', $labChecklist) }}" class="space-y-4" onsubmit="return window.arcadiaCheckSignaturePresent(this, 'checklist-approve-pad_input')">
                        @csrf
                        ...
                        <x-lab-signature-pad id="checklist-approve-pad" />
                    ...
                    <form method="POST" action="{{ route('lab-checklists.reject', $labChecklist) }}" class="mt-4 space-y-4 border-t border-slate-100 pt-4">
                        @csrf
            ...
            @can('unlock', $labChecklist)
                <div class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
                    <p class="text-sm text-slate-600">Reopen this checklist for editing.</p>
                    <form method="POST" action="{{ route('lab-checklists.unlock', $labChecklist) }}" class="mt-4">@csrf<button class="rounded-xl bg-amber-500 px-5 py-2.5 text-sm font-semibold text-white shadow-sm hover:bg-amber-600">Unlock</button></form>
                </div>
            @endcan
