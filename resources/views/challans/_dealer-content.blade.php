{{-- Master Dealer Challan body — shared by challans.dealer (standalone) and
     challans.print-all (combined with every farmer challan). Expects
     $challan (type=dealer), $dispatch, $lines (this dealer's DispatchLine
     rows for the dispatch) and optional $isPublic. --}}
...
<table class="header">
    <tr>
        ...
        <td class="meta-cell">
            <div class="doc-title">DEALER CHALLAN</div>
            <div class="meta-line"><strong>DC No:</strong> {{ $challan->challan_no }}</div>
...
{{-- ============ DEALER + TRANSPORT DETAILS ============ --}}
<table class="two-col">
    <tr>
        <td class="left-pad">
            <div class="box">
                <div class="box-title">Dealer Details</div>
                <table class="kv">
                    <tr><td class="k">Dealer name</td><td class="v">{{ $challan->dealer->dealer_name }}</td></tr>
                    <tr><td class="k">Address</td><td class="v">{{ collect([$challan->dealer->address, $challan->dealer->village?->name, $challan->dealer->taluka?->name, $challan->dealer->district?->name, $challan->dealer->pin_code])->filter()->implode(', ') ?: '—' }}</td></tr>
                    <tr><td class="k">Mobile</td><td class="v">{{ $challan->dealer->mobile ?? '—' }}</td></tr>
                </table>
            </div>
        </td>
        <td class="right-pad">
            <div class="box">
                ...
                <table class="kv">
                    ...
                    <tr><td class="k">Marketing officer</td><td class="v">{{ $dispatch->vehicleAssignment?->marketingOfficer?->name ?? '—' }}</td></tr>
                    <tr><td class="k">Route</td><td class="v">{{ $dispatch->vehicleAssignment?->dispatchPlan?->route ?? '—' }}</td></tr>
...
<table class="items">
    <thead>
        <tr>
            <th style="width:34%">Farmer</th>
            <th style="width:28%">Village</th>
            <th style="width:20%">Variety</th>
            <th class="num" style="width:18%">Quantity</th>
        </tr>
    </thead>
    <tbody>
        @foreach($lines->groupBy('farmer_id') as $farmerLines)
            @php($farmer = $farmerLines->first()->farmer)
            <tr>
                <td>{{ $farmer?->farmer_name }}</td>
                <td>{{ $farmer?->village?->name ?? '—' }}</td>
                <td>{{ $farmerLines->pluck('booking.variety')->filter()->unique()->implode(', ') }}</td>
                <td class="num">{{ number_format($farmerLines->sum('total_qty')) }}</td>
            </tr>
        @endforeach
    </tbody>
    <tfoot>
        <tr>
            <td colspan="3">Grand Total Plants</td>
    ...
    <tr>
        <td class="left-pad">
            <div class="box">
                ...
                <table class="kv">
                    <tr><td class="k">Farmers on this challan</td><td class="v">{{ $lines->pluck('farmer_id')->unique()->count() }}</td></tr>
