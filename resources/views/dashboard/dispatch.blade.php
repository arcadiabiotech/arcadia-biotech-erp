<x-app-layout>
    <div class="mx-auto max-w-7xl">
        <div class="mb-8">
            <p class="mb-1 text-sm font-semibold text-blue-600">DISPATCH</p>
            <h1 class="text-2xl font-bold tracking-tight text-slate-900 sm:text-3xl">Welcome back, {{ auth()->user()->name }}</h1>
            <p class="mt-2 text-sm text-slate-500">Approved bookings ready to ship, and your live dispatch pipeline.</p>
        </div>
        ...
        <div class="grid gap-6 lg:grid-cols-2">
            <section class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
                <div class="mb-5 flex items-center justify-between">
                    <div>
                        <h2 class="text-lg font-bold text-slate-900">Approved Bookings</h2>
                        <p class="mt-1 text-sm text-slate-500">Ready to be dispatched.</p>
                    </div>
                ...
                <div class="divide-y divide-slate-100">
                    @forelse($approvedBookings as $booking)
                        <div class="flex items-center justify-between gap-3 py-3">
                            ...
                            <span class="shrink-0 text-xs font-semibold text-slate-500">{{ number_format($booking->plant_qty) }} plants</span>
                        </div>
                    @empty
                        <p class="py-8 text-center text-sm text-slate-500">No approved bookings waiting right now.</p>
                    @endforelse
                </div>
            </section>
            ...
            <section class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
                <div class="mb-5 flex items-center justify-between">
                    <div>
                        <h2 class="text-lg font-bold text-slate-900">Dispatch</h2>
                        <p class="mt-1 text-sm text-slate-500">Latest shipments in the pipeline.</p>
                    </div>
                    ...
                    @forelse($recentDispatches as $dispatch)
                        <div class="flex items-center justify-between gap-3 py-3">
                            <div class="min-w-0">
                                ...
                                <p class="truncate text-xs text-slate-500">{{ $dispatch->lines->pluck('dealer.dealer_name')->filter()->unique()->implode(', ') }} · {{ $dispatch->vehicle?->vehicle_no ?? 'No vehicle yet' }}</p>
                            </div>
                            <x-dispatch-status-badge :status="$dispatch->status" class="shrink-0" />
                        </div>
                    @empty
                        <p class="py-8 text-center text-sm text-slate-500">No dispatches yet.</p>
                    @endforelse
                </div>
            </section>
        </div>
