<x-app-layout>
    <div class="mx-auto max-w-7xl">
        <div class="mb-8">
            <p class="mb-1 text-sm font-semibold text-blue-600">DEALER</p>
            <h1 class="text-2xl font-bold tracking-tight text-slate-900 sm:text-3xl">Welcome back, {{ auth()->user()->name }}</h1>
            <p class="mt-2 text-sm text-slate-500">Your farmers, bookings and dispatch status at a glance.</p>
        </div>
        ...
        <div class="grid gap-6 lg:grid-cols-2">
            <section class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
                <div class="mb-5 flex items-center justify-between">
                    <div>
                        <h2 class="text-lg font-bold text-slate-900">My Bookings</h2>
                        <p class="mt-1 text-sm text-slate-500">Your most recent plant bookings.</p>
                    </div>
                    ...
                    @forelse($recentBookings as $booking)
                        <div class="flex items-center justify-between gap-3 py-3">
                            <div class="min-w-0">
                                ...
                                <p class="truncate text-xs text-slate-500">{{ $booking->farmer?->farmer_name }}</p>
                            </div>
                            <x-booking-status-badge :status="$booking->approval_status" class="shrink-0" />
                        </div>
                    @empty
                        <p class="py-8 text-center text-sm text-slate-500">You have no bookings yet.</p>
                    @endforelse
                </div>
            </section>
            ...
            <section class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
                <div class="mb-5 flex items-center justify-between">
                    <div>
                        ...
                        <p class="mt-1 text-sm text-slate-500">Your most recent shipments.</p>
                    </div>
                    ...
                    @forelse($recentDispatches as $dispatch)
                        <div class="flex items-center justify-between gap-3 py-3">
                            <div class="min-w-0">
                                ...
                                <p class="truncate text-xs text-slate-500">{{ $dispatch->lines->pluck('booking.booking_no')->filter()->implode(', ') }} · {{ $dispatch->vehicle?->vehicle_no ?? 'No vehicle yet' }}</p>
                            </div>
